...
const UpdateCPTSchema = z.object({
...
const updateCPTCodesHandler = async (event: APIGatewayProxyEvent, context: Context) => {
  const startTime = Date.now();
  const requestId = context.awsRequestId;

  logger.info('[Update CPT Codes] Handler started', {
    requestId,
    path: event.path,
  ...
  const { codes } = validateBody(event, UpdateCPTSchema);
  ...
  try {
    ...
    logger.info('[Update CPT Codes] CPT codes updated successfully', {
      noteId,
      providerId,
    ...
    const duration = Date.now() - startTime;
    logger.error('[Update CPT Codes] Failed to update CPT codes', {
      noteId,
      providerId,
...
export const handler = errorHandler(updateCPTCodesHandler);